...
contains
   ...
   subroutine do_debug_ast(flags, pattern)
      ...
      character(:),allocatable :: ast, prefix, postfix, entire
      ...
      output: block
         character(NUM_DIGIT_KEY) :: parse_time, literal_time, tree_count, tree_allocated, &
            memory, literal_pre, literal_post, literal_all
         character(NUM_DIGIT_KEY) :: cbuff(8)
         integer :: i
         ...
         if (flags(FLAG_VERBOSE)) then
            cbuff = [parse_time, literal_time, literal_all, literal_pre, literal_post, memory, tree_count, tree_allocated]
            ...
            write(stdout, fmt_out_char) trim(cbuff(5)), postfix
            write(stdout, fmt_out_int) trim(cbuff(6)), mem_tape(tree%tape) + mem_tree(tree%nodes)
            write(stdout, fmt_out_int) trim(cbuff(7)), root
            write(stdout, fmt_out_int) trim(cbuff(8)), size(tree%nodes, dim=1)
         else if (flags(FLAG_NO_TABLE)) then
            continue
         else
            cbuff = [parse_time, literal_time, literal_all, literal_pre, literal_post, memory, (repeat(" ", NUM_DIGIT_KEY), i=1, 2)]
            call right_justify(cbuff)
